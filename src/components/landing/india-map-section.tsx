...
type MapMode = 'NATIONWIDE' | 'LOCALLY'
...
const localHubs = [
  { name: 'New Delhi', x: 286, y: 184 },
  { name: 'Mumbai', x: 227, y: 396 },
  { name: 'Bengaluru', x: 287, y: 601 },
  { name: 'Chennai', x: 341, y: 622 },
  { name: 'Kolkata', x: 487, y: 339 },
]

function generateGridPoints(): { x: number; y: number }[] {
...
export function IndiaMapSection() {
  const [mode, setMode] = useState<MapMode>('NATIONWIDE')
  ...
  useEffect(() => {
    if (!rendered.current) {
      ...
      setGridDots(generateGridPoints())
    }
  }, [])

  useEffect(() => {
    const interval = setInterval(() => {
      setMode((prev) => (prev === 'NATIONWIDE' ? 'LOCALLY' : 'NATIONWIDE'))
    }, 6000)
    return () => clearInterval(interval)
  }, [])

  return (
    <section className="py-16 relative overflow-hidden" style={{ backgroundColor: '#F8FAFC' }}>
      <div className="max-w-5xl mx-auto px-6">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
          ...
          >
            <h2 className="text-4xl md:text-5xl font-bold tracking-tight leading-tight" style={{ color: '#0F172A' }}>
              Deliver Nationwide <br />
              <span style={{ color: '#2563EB' }}>or Locally</span>
            </h2>
            <p className="text-base leading-relaxed" style={{ color: '#64748B' }}>
              Reach target audiences cross-country or zero-in on specific regional communities seamlessly across premium streaming environments.
            </p>
            <a href="/campaign/new" className="inline-flex items-center text-sm font-medium hover:underline pt-2 group" style={{ color: '#2563EB' }}>
              Get started
              <span className="transform translate-x-0 group-hover:translate-x-1 transition-transform ml-1">&rarr;</span>
            </a>
          </motion.div>
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            className="relative w-full max-w-[480px] aspect-[5/6] rounded-2xl p-6 flex items-center justify-center overflow-hidden"
            style={{ backgroundColor: '#FFFFFF', border: '1px solid #E2E8F0', boxShadow: '0 1px 3px rgba(0,0,0,0.06), 0 1px 2px rgba(0,0,0,0.04)' }}
          >
            <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} className="w-full h-full transition-transform duration-700 ease-out">
              <AnimatePresence>
                {mode === 'NATIONWIDE' && (
                  <>
                    {[1, 2, 3].map((circle) => (
                      <motion.circle
                        ...
                        className="fill-none"
                        style={{ stroke: '#2563EB', strokeOpacity: 0.12 }}
                        strokeWidth="1.5"
                      />
                    ))}
                  </>
              ...
              <g id="india-dot-matrix">
                {gridDots.map((pt, i) => {
                  const isNearHub = localHubs.some(
                    (hub) => Math.hypot(hub.x - pt.x, hub.y - pt.y) < 38
                  )
                  return (
                    <motion.circle
                      key={i}
                      ...
                      cy={pt.y}
                      animate={{
                        fill: mode === 'NATIONWIDE'
                          ? '#2563EB'
                          : isNearHub ? '#FF007A' : 'rgba(148, 163, 184, 0.15)',
                        r: mode === 'NATIONWIDE' ? DOT_R : DOT_R * 0.85,
                      }}
            ...
            <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2">
              <div className="rounded-full px-5 py-1.5 flex items-center justify-center"
              ...
              >
                <motion.span
                  ...
                  style={{ color: mode === 'NATIONWIDE' ? '#2563EB' : '#FF007A' }}
                >
                  {mode}
                </motion.span>
              </div>
