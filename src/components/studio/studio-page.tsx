...
import { StateProvider, useStudioState } from '@/lib/state-context'
...
function StudioContent() {
  ...
  if (showEditor) {
    return <EditorModal onClose={() => setShowEditor(false)} />
  }
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-[#0a0a0f] flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-3xl mx-auto px-6 text-center">
        ...
        <p className="text-base text-white/60 mb-8 max-w-lg mx-auto">
          Enter a URL and we&apos;ll auto-generate a full video ad with slides, script, and branding.
        </p>
        ...
        <div className="relative max-w-xl mx-auto">
          ...
          {showDropdown && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-[#1a1a25] border border-white/10 rounded-xl overflow-hidden shadow-2xl z-50">
              <div className="px-4 py-2 text-[10px] text-white/30 uppercase tracking-widest font-semibold">Via URL</div>
              <button
      ...
      {showGenerating && (
        <div className="fixed inset-0 z-[100] bg-[#0a0a0f] flex flex-col items-center justify-center">
          ...
          <div className="w-72 space-y-2">
            {logs.map((log, i) => (
              <div key={i} className={`flex items-center gap-2 text-xs font-mono ${i === logs.length - 1 ? 'text-accent' : 'text-white/40'}`}>
